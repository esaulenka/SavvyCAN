...
SerialBusConnection::SerialBusConnection(QString portName, QString driverName, int pBusSpeed, int pDataRate, bool pCanFd) :
    CANConnection(portName, driverName, CANCon::SERIALBUS, 0, pBusSpeed, pCanFd, pDataRate, 1, 4000),
...
void SerialBusConnection::disconnectDevice() {
    if (mDev_p &&
        mDev_p->state() == QCanBusDevice::ConnectedState) {
    ...
    while(true)
    {
        ...
        if (true) {
            ...
            if(frame_p) {
                ...
                quint64 timestamp = useSystemTime ?
                    QDateTime::currentMSecsSinceEpoch() * 1000ul :
                    recFrame.timeStamp().seconds() * 1000000ul + recFrame.timeStamp().microSeconds();
                frame_p->setTimeStamp(CommFrame::TimeStamp::fromMicroSeconds(timestamp));
